...
%% The public Logging methods %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function writeTurnAndPlayer(self,scriptName,message)
        
            %self.writeLog(self.INFO,funcName,message);
            % This records the
            % turn taken and player number -- this will be called from the Game
            % class when initializing destination cards and when announcing
            % the current turn and player during the game
            try
                fid = fopen(self.fullpath,'a');
                fprintf(fid,'%s - %s\r\n' ...
                    , scriptName ... % Have left this one with the '.' if it is passed
        ...
        function writePlayerTurnDetails(self,scriptName,message)
        
            %self.writeLog(self.INFO,funcName,message);
            % This function is used to provide specific details 
            % about a player's move such as cards chosen or routes
            % claimed -- this function was written in order to create
            % add an extra tab to help with formatting the log
            try
                fid = fopen(self.fullpath,'a');
                fprintf(fid,'\t%s - %s\r\n' ...
                    , scriptName ... % Have left this one with the '.' if it is passed
